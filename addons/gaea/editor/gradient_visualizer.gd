...
func _ready() -> void:
	...
	tooltip_text = "Color used is the [GaeaMaterial]'s [param preview_color]"
...
func update() -> void:
	...
	for idx: int in gradient.points.size():
		...
		print_rich(start_offset  * SIZE.x, " to ", end_offset  * SIZE.x, " with [color=%s]" % color.to_html(), color, "[/color]")
	...
	image.save_png("res://scenes/test/test.png")
	print_rich("[img]res://scenes/test/test.png[/img]")
