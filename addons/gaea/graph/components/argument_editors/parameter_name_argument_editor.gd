...
func _on_line_edit_text_changed(new_text: String, line_edit: LineEdit) -> void:
	var editor_interface = Engine.get_singleton("EditorInterface")
	if (graph_node._graph_edit.graph.has_parameter(new_text) and new_text != _name_label.text) or not new_text.is_valid_identifier():
...
func _on_line_edit_text_submitted(new_text: String, line_edit: LineEdit) -> void:
	...
	if graph_node._graph_edit.graph.has_parameter(new_text):
